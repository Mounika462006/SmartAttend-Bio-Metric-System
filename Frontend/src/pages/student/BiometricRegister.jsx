...
export default function BiometricRegister() {
  ...
  }, [descriptors, capturedImages]);

  const submitBiometric = async (firstImg, secondImg, descriptor, similarity) => {
    setLoading(true);
    try {
      ...
      setError(err.response?.data?.message || 'Failed to save biometric data. Please try again.');
      setStep(1);
    } finally {
      setLoading(false);
    }
